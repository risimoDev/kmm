-- ═══════════════════════════════════════════════════════════
-- Контент Завод — Миграция: Анализатор + Видео-Композитор
-- Запускать: psql -U $DB_USER -d $DB_NAME -f migration-composer.sql
-- ═══════════════════════════════════════════════════════════

-- ─────────────────────────────────────────
-- 1. Вирусные референсы (YouTube / TikTok)
-- ─────────────────────────────────────────
...
-- ─────────────────────────────────────────
-- 2. Стиль-паки для видео-монтажа
-- ─────────────────────────────────────────
CREATE TABLE IF NOT EXISTS style_packs (
  ...
  subtitle_primary_color  VARCHAR(20)  DEFAULT '&H00FFFFFF',  -- &HAABBGGRR (ASS)
  ...
  -- Цветовая обработка (FFmpeg filter)
  color_filter            TEXT         DEFAULT '',
  vignette                BOOLEAN      DEFAULT FALSE,

  -- Параметры кодирования
  output_quality          INTEGER      DEFAULT 23,  -- CRF: ниже = лучше
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_style_packs_default ON style_packs(is_default) WHERE is_default = TRUE;

-- ─────────────────────────────────────────
-- 3. Дефолтные стиль-паки (6 вариантов)
-- ─────────────────────────────────────────
